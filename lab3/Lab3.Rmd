...
$R^{2}_{nowe}$= `r toString(R_2_2)` \> `r toString(R_2)`=$R^{2}_{stare}$
...
```{r zad_7, echo=FALSE, eval=TRUE, results='asis'}


obserwowane_DC<-DC
DC_w_starym_modelu<-predict(model)
DC_w_nowym_modelu<-predict(model2)
tabela<-data.frame(obserwowane_DC,DC_w_starym_modelu,DC_w_nowym_modelu)
xtab<-xtable(tabela)

print(xtab, type = "latex", table.placement = "H", comment=FALSE)
